    ...
    bool is_movement_valid(coordinates origin, coordinates target, const vector<vector<Piece *>> &board) override {
        ...
        if(delta_x == 0 && abs(delta_y) < 3) {
            ...
            if(abs(delta_y) == 2 && !has_moved) {
                if(board[origin.x][origin.y + step] != nullptr) return false;
                if(board[origin.x][origin.y + 2*step] != nullptr) return false;
                else {
                    has_moved = true;
                    return true;
                }
        ...
        if(abs(delta_y) == 1 && abs(delta_x) == 1) {
            if(board[target.x][target.y] == nullptr) return false;
            if(board[target.x][target.y]->getis_white() == getis_white()) return false;
            if(target.y == 7) {
            }
            return true;
